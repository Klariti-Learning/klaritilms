...
useEffect(() => {
  const fetchUserDetails = async () => {
    ...
    try {
      ...
      console.error("[TeacherProfile] Error fetching user details:", apiError);
      if (apiError.response?.status === 401) {
        handleUnauthorized();
      } else {
        toast.error(apiError.response?.data?.message || "Error fetching user details");
      }
    } finally {
  ...
  useEffect(() => {
    if (
      !authLoading &&
      (!user || !user.role || user.role.roleName !== "Teacher")
    ) {
  ...
  useEffect(() => {
    if (userDetails) {
      setFormData({
        ...
        role: userDetails.role || { roleName: "Teacher" },
...
const handleSave = async () => {
  ...
  try {
    ...
    const updatedFormData = {
      ...
      role: {
        roleName:
          userDetails?.role?.roleName || formData.role?.roleName || "Teacher",
      },
    };
    ...
    const apiError = error as ApiError;
    console.error("[TeacherProfile] Error updating profile:", apiError);
    if (apiError.response?.status === 401) {
      handleUnauthorized();
    } else {
      toast.error(apiError.response?.data?.message || "Error updating profile");
    }
  } finally {
    setIsSaving(false);
  }
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <Input
                                ...
                                placeholder="Enter title (e.g., Senior Teacher)"
