...
export default function StudentProfile() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      try {
        ...
        console.error("[StudentProfile] Error fetching user details:", {
          errorMessage,
          errors:
    ...
    fetchUserDetails();
  }, [user, deviceId, setUserDetails, handleUnauthorized]);

  useEffect(() => {
    const fetchTeacherName = async () => {
      if (!userDetails?.teacherId || !deviceId) {
      ...
      try {
        ...
        const response = await api.get(
          `/auth/teacher/${userDetails.teacherId}`,
          {
            headers: { "Device-Id": deviceId },
          }
        );
        setAssignedTeacher(response.data.teacher.name);
      } catch (error) {
        const errorMessage = error as ApiError;
        console.error("[StudentProfile] Error fetching teacher:", {
          errorMessage,
          errors:
            errorMessage.response?.data?.message ||
    ...
    fetchTeacherName();
  }, [userDetails, deviceId, handleUnauthorized]);
  ...
  const calculateCompletion = (details: UserDetails | null) => {
    ...
    const fields = [
      ...
      details.address,
      details.timezone,
      details.preferredTimeSlots?.length ? details.preferredTimeSlots : null,
      details.studentId,
      details.subjects?.length ? details.subjects : null,
  ...
  useEffect(() => {
    if (
      !authLoading &&
      (!user || !user.role || user.role.roleName !== "Student")
    ) {
      handleUnauthorized();
    }
  ...
  const confirmArrayItem = (
    ...
    const singularFieldNames: { [key: string]: string } = {
      hobbies: "hobby",
      skills: "skill",
    ...
    setFormData((prev) => {
      ...
      if (subField) {
        let value = "";
        if (subField === "hobbies" && pendingHobbies[index]) {
          value = pendingHobbies[index];
        } else if (subField === "skills" && pendingSkills[index]) {
          value = pendingSkills[index];
        } else if (
          subField === "accomplishments" &&
          pendingAccomplishments[index]
        ) {
          value = pendingAccomplishments[index];
        } else if (
          subField === "qualifications" &&
          pendingQualifications[index]
        ) {
          value = pendingQualifications[index];
        }
            ...
            [subField]: prev[subField].map((err, i) =>
              i === index
                ? `Please enter a valid ${
                    singularFieldNames[subField] || subField
                  }.`
                : err
            ),
          }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatedFormData = {
        ...
        role: {
          roleName:
            userDetails?.role?.roleName || formData.role?.roleName || "Student",
        },
      };
      ...
      console.error("[StudentProfile] Error updating profile:", {
        errorMessage,
        errors:
          errorMessage.response?.data?.message || "Error updating profile",
      });
      if (errorMessage.response?.status === 401) {
        handleUnauthorized();
      } else {
  ...
  return (
    <motion.div
      ...
      className="min-h-screen w-full  bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8"
    >
      <div
      ...
      >
        ...
        {userDetails.studentId && (
          <motion.div
            ...
            className={`absolute top-4 right-4 px-4 py-2 rounded-lg shadow-lg font-semibold text-white ${
              isEditing
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <div className="flex-1 ml-4">
                  ...
                  <p className="mt-1 text-gray-800 text-lg font-medium">
                    {(userDetails.preferredTimeSlots || []).join(", ") ||
                      "Not set"}
                  </p>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="flex-1 ml-4">
                  ...
                  {isEditing ? (
                    ...
                    >
                      <Select
                        value={formData.profile?.enrollmentStatus || ""}
                      ...
                      >
                        <SelectTrigger
                          className={`mt-1 border-indigo-300 bg-white text-gray-800 rounded-xl focus:ring-2 focus:ring-indigo-400 hover:bg-indigo-50 transition-all duration-300 hover:scale-[1.01] shadow-sm ${
                            isEditing ? "ring-1 ring-indigo-200" : ""
                          }`}
                        >
                          <SelectValue
                        ...
                        <SelectContent className="bg-white border-indigo-200 rounded-xl shadow-lg">
                          ...
                          >
                            <SelectItem
                              value="Active"
                              className="hover:bg-indigo-50 text-gray-800 transition-colors duration-200"
                            >
                              Active
                            </SelectItem>
                            <SelectItem
                    ...
                    <p className="mt-1 text-gray-800 text-lg font-medium">
                      {userDetails.profile?.enrollmentStatus || "Not set"}
                    </p>
                  )}
                </div>
              </motion.div>
                ...
                <div className="flex-1 ml-4">
                  ...
                  {isEditing ? (
                    <Input
                      ...
                      onChange={(e) =>
                        handleInputChange(e, "profile", "academicYear")
                      }
                  ...
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      <Button
                        ...
                        className="mt-2 bg-indigo-500 hover:bg-indigo-600 transition-all duration-200 hover:scale-105 cursor-pointer text-white"
                      >
                        Add Hobby
                      </Button>
                    </div>
