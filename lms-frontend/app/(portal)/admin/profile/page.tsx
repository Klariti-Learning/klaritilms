...
export default function AdminProfile() {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      ...
      console.error("[AdminProfile] Error fetching user details:", apiError);
...
useEffect(() => {
  ...
  useEffect(() => {
    if (userDetails) {
      setFormData({
        ...userDetails,
        role: userDetails.role || { roleName: "Admin" },
  ...
  const handleSave = async () => {
    ...
    if (!token || !deviceId) {
      ...
      const updatedFormData = {
        ...
        role: {
          roleName:
            userDetails?.role?.roleName || formData.role?.roleName || "Admin",
        },
      };
    ...
    const apiError = error as ApiError;
    console.error("[AdminProfile] Error updating profile:", apiError);
    if (apiError.response?.status === 401) {
      handleUnauthorized();
    } else {
