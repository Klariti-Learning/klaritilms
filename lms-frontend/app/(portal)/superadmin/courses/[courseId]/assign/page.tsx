  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      ...
      try {
        ...
        console.error("[AssignTeachers] Failed to fetch teachers:", apiError);
  ...
  useEffect(() => {
    const fetchAssignedTeachers = async () => {
      try {
        ...
        const apiError = error as ApiError;
        console.error(
          "[AssignTeachers] Failed to fetch assigned teachers:",
          apiError
        );
        if (apiError.response?.status === 401) {
          handleUnauthorized();
          ...
          toast.error(
            apiError.response?.data?.message ||
              "Failed to fetch assigned teachers"
          );
        }
      }
    };
  ...
  const handleAssignTeachers = async () => {
    ...
    try {
      ...
      if (apiError.response?.status === 401) {
        ...
        toast.error(
          apiError.response?.data?.message || "Failed to assign course"
        );
      }
    } finally {
