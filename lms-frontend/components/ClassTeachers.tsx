...
import { Users, Calendar, Clock, Award, ChevronRight, ChevronDown, X, Download } from 'lucide-react';
...
const ClassTeachers: React.FC = () => {
  const [selectedTeacher, setSelectedTeacher] = useState<MappedTeacher | null>(null);
  ...
  const isJoinButtonEnabled = (teacher: MappedTeacher) => {
    if (!teacher.nextClassDate || teacher.nextClassDate === 'N/A' || !teacher.nextClassTime) {
      return false;
    }
    ...
    try {
      ...
      const timeDiff = (classDateTime.getTime() - currentTime.getTime()) / (1000 * 60);
  ...
  const fetchAttendanceData = async (batchId: string, batchName: string, fromDate?: string, toDate?: string) => {
    try {
      setModalLoading(true);
      const params: { batchId: string; fromDate?: string; toDate?: string } = { batchId };
      if (fromDate && toDate) {
  ...
  const exportToCSV = async () => {
    try {
      const params: { batchId: string; fromDate?: string; toDate?: string } = { batchId: selectedBatchId };
      if (fromDate && toDate) {
        params.fromDate = fromDate;
        params.toDate = toDate;
      }
      ...
      const csvRows = [
        ...
        ...records.map((record: AttendanceRecord) => {
          ...
          const course = record.course?.title || 'N/A';
          const batch = record.batch?.name || 'N/A';
          const teacher = record.teacher?.name || 'N/A';
          const student = record.students[0]?.name || 'N/A';
          const status = record.students[0]?.status || 'N/A';
          return `"${date}","${course}","${batch}","${teacher}","${student}","${status}"`;
        }),
      ];
      ...
      link.download = `${selectedBatchName}_attendance.csv`;
      link.click();
    } catch (err) {
      ...
      try {
        ...
        await Promise.all(
          batches.map(async (batch) => {
            try {
              const callsResponse = await api.get(`/schedule/batch/${batch._id}/calls`);
              const batchData: BatchCallResponse['batch'] = callsResponse.data.batch;
              const calls = batchData.calls || [];
              const currentDate = new Date();

              const schedule = calls.length > 0
                ? [...new Set(calls.flatMap((call) => call.days))].map((day) => {
                    ...
                    const startTime = call.startTime ? formatTime(call.startTime) : 'N/A';
                    const endTime = call.endTime ? formatTime(call.endTime) : 'N/A';

                    return `${shortDay} - ${startTime} - ${endTime}`;
                  })
                : ['N/A'];

              const firstClassDate = calls.length > 0
                ? new Date(Math.min(...calls.map((call) => new Date(call.date).getTime())))
                    .toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })
                : 'N/A';

              const totalAttended = calls.length > 0
                ? `${calls.filter((call) => call.status === 'Completed').length} / ${calls.length}`
                : 'N/A';

              const nextCall = calls.find(
                (call) => call.status === 'Scheduled' && new Date(call.date) > currentDate
              );
              ...
              const nextClassTime = nextCall && nextCall.startTime
                ? `${new Date(nextCall.date).toLocaleDateString('en-US', {
                    day: '2-digit',
                    month: 'short',
                    year: 'numeric',
              ...
              const teacherName = calls[0]?.teacherName || batchData.teacherName || 'Unknown Teacher';
              ...
              const teacher: MappedTeacher = {
                ...
                initials:
                  teacherName
                    ...
                    .toUpperCase() || 'UT',
                subject: batch.name,
                courseTitle: batch.courseTitle || batchData.courseTitle || 'N/A',
              ...
              if (teacherMap.has(teacherName)) {
                ...
                existingTeacher.schedule = [...new Set([...existingTeacher.schedule, ...schedule])];
                existingTeacher.subjects = [...new Set([...existingTeacher.subjects, ...teacher.subjects])];
                existingTeacher.courseTitle = `${existingTeacher.courseTitle}, ${teacher.courseTitle}`;
              } else {
                teacherMap.set(teacherName, teacher);
              }
            } catch (callErr) {
              console.error(`[ClassTeachers] Error fetching calls for batch ${batch._id}:`, callErr);
              const teacher: MappedTeacher = {
                id: batch._id,
      ...
      <div className="max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
        <div className="p-6 border-b border-gray-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Class Teachers</h2>
              <p className="text-base text-gray-600">Guiding students every step of the way</p>
            </div>
          </div>
                    ...
                    <div className="w-12 h-12 rounded-full flex items-center justify-center overflow-hidden">
                      {teacher.profileImage ? (
                        ...
                        <div className="w-full h-full bg-blue-500 flex items-center justify-center">
                          <span className="text-white font-bold text-base">{teacher.initials}</span>
                        </div>
                      )}
                    </div>
                    <div>
                  ...
                  <div className="mt-3 p-6 bg-gray-50 rounded-2xl border border-gray-200">
                    <div className="mb-5">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-4">
                          <div>
                            <h3 className="font-bold text-sm text-gray-900">{teacher.subject}</h3>
                            <p className="text-xs text-gray-600">{teacher.courseTitle}</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-5">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-blue-600 whitespace-nowrap">Schedule</span>
                          </div>
                          <div className="text-xs text-gray-700 space-y-2">
                            {teacher.schedule.map((time: string, index: number) => (
                              <p key={index} className="font-medium whitespace-nowrap">
                                {time}
                              </p>
                            ))}
                          </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <Calendar className="w-3 h-3 text-blue-600" />
                            <span className="text-xs font-semibold text-blue-600 whitespace-nowrap">First Class</span>
                          </div>
                          <div className="text-xs text-gray-700">
                            <p className="font-medium">{teacher.firstClassDate}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-blue-600 whitespace-nowrap">Attended</span>
                          </div>
                          <div className="text-xs text-gray-700">
                            <p className="font-medium">{teacher.totalAttended}</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-blue-600 whitespace-nowrap">Next Class</span>
                          </div>
                          <div className="text-xs text-gray-700">
                            <p className="font-medium">{teacher.nextClassDate}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          onClick={() => handleOpenAttendance(teacher.id, teacher.subject)}
                        >
                          Attendance
                        </button>
                        <Link
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={`w-full justify-start text-left font-normal border-2 rounded-lg transition-all ${
                          fromDate ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
                        } hover:border-blue-500 focus:ring-2 focus:ring-blue-300`}
                      >
                        <Calendar className="mr-2 h-4 w-4 text-blue-600" />
                        {fromDate ? format(parseISO(fromDate), 'PPP') : <span className="text-gray-500">Pick a date</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 border-2 border-blue-200 rounded-lg shadow-lg">
                      <CalendarComponent
                        ...
                        modifiers={{
                          selected: (date: Date) =>
                            fromDate ? format(parseISO(fromDate), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd') : false,
                        }}
                        modifiersClassNames={{
                          selected: 'bg-blue-100 border-2 border-blue-500 rounded-full',
                        }}
                      />
                    </PopoverContent>
                  </Popover>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        className={`w-full justify-start text-left font-normal border-2 rounded-lg transition-all ${
                          toDate ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
                        } hover:border-blue-500 focus:ring-2 focus:ring-blue-300`}
                      >
                        <Calendar className="mr-2 h-4 w-4 text-blue-600" />
                        {toDate ? format(parseISO(toDate), 'PPP') : <span className="text-gray-500">Pick a date</span>}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 border-2 border-blue-200 rounded-lg shadow-lg">
                      <CalendarComponent
                        ...
                        modifiers={{
                          selected: (date: Date) =>
                            toDate ? format(parseISO(toDate), 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd') : false,
                        }}
                        modifiersClassNames={{
                          selected: 'bg-blue-100 border-2 border-blue-500 rounded-full',
                        }}
                ...
                <div className="flex gap-2 mt-5">
                  ...
                  <Button
                    ...
                    disabled={modalLoading || attendanceRecords.length === 0}
                  >
            ...
            <div className="flex-1 overflow-y-auto">
              {modalLoading ? (
                ...
                <p className="text-gray-600 text-center p-6">No attendance records available.</p>
              ) : (
